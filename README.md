...
[![Version](https://img.shields.io/badge/version-v0.3.4-blue.svg)](https://pypi.org/project/harmonsmile/)
...
* **PubChem acquisition provenance**: Records whether a usable PubChem
  property row was obtained for each CID through
  `PubChem_Acquisition_Status` and `PubChem_Acquisition_Message`, independently
  of subsequent molecular harmonization.
...
## PubChem Acquisition Contract

`PubChemIngest` reports the outcome of PubChem property acquisition separately
from molecular harmonization through two row-level fields:

- `PubChem_Acquisition_Status`: machine-readable acquisition outcome.
- `PubChem_Acquisition_Message`: optional diagnostic message associated with
  that outcome.

`PubChem_Acquisition_Status` uses three values:

- `ok`: a usable PubChem property row was obtained. Individual requested
  properties may still be absent or null.
- `failed`: no usable PubChem property row was obtained within the configured
  acquisition attempt budget for that invocation.
- `not_attempted`: no PubChem HTTP acquisition was attempted because the local
  CID was missing or invalid after local handling.

`PubChem_Acquisition_Message` is intended for diagnostic and audit purposes.
Programmatic decisions should use `PubChem_Acquisition_Status`.

PubChem acquisition and molecular harmonization describe different stages of
the pipeline. An acquisition status of `ok` therefore does not imply that
`SMILES_Harmonization_Status` is also `ok`. For example, acquisition may
succeed while the returned SMILES is absent or cannot be harmonized.

Conversely, a harmonization failure does not by itself establish that PubChem
acquisition failed. `PubChem_Acquisition_Status` is the authoritative field for
that distinction.
...
The returned DataFrame includes `PubChem_Acquisition_Status` and
`PubChem_Acquisition_Message`, which record PubChem acquisition outcome
separately from the molecular harmonization fields.
...
`SMILES_Harmonization_Status`, and `SMILES_Harmonization_Message`.
`PubChemIngest` additionally reports PubChem acquisition provenance through
...
|-- tests/                 # Unit test suite (pytest) - 196 tests
